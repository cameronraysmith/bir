...
Colimits over the category of elements of a presheaf are used in the construction of the functors $R$ and $L$ informally presented in Figure \ref{fig:ascent}.
...
$$
which commutes uniquely in $L$.
\iftoggle{thmsty}{
\end{proof}
}

In order to be able to represent objects in the global category of interacting biological entities in terms of diagrams modeled in the local category we make use of a very general notion of topology that can be applied to any category and that can be specialized to, for example, the case in which objects and morphisms of the category take on a particular type of algebraic structure such as rings or graphs among others. The purpose of this construction is to develop a method of defining a covering system composed of interacting entities in the category representing local information that can be used to establish a decomposition of objects in the category representing global information with respect to a biological system.
