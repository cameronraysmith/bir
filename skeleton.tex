...
\title{\bf Intrinsic operationalism in biological systems}
...
\date{\today}
\begin{abstract}
Operationalism was introduced in physics as a means of associating meaning to concepts in terms of methods of measurement. In biological systems, collections of chemical interactions and transformations that molecules participate in or perform upon one another can be viewed as constituting a form of {\it intrinsic operationalism} where measurement outcomes may span the range from random fluctuations to robust correlations. The basis for this point of view is that information can be transferred from one type of molecule to another target via a third only if the intermediary is capable of intrinsically correlating some interaction or reaction with or to the first with another interaction or reaction to the target. Many biological functions depend on such information preserving pathways of interaction while their {\it a priori} likelihood of existence is small relative to that of pathways that are incapable of such information preservation. The stability of such information preserving pathways relies on the absence of additional modes of interaction on the part of any of the pathway constituents that could interfere with what would otherwise represent robust correlations. This scenario results in two constraints: one as an upper bound on the number of different types of molecules that can simultaneously participate in independent information preserving pathways in a strongly coupled environment and another as a direct result of the first which is an upper bound on the number of different biological functions capable of being performed by the information preserving pathways involved. When these constraints become relevant to evolutionary processes, any mechanism capable of circumventing them may require substantial innovation.
\end{abstract}
