...
\caption{As a concrete depiction in terms of graphs for intuitive consideration, but not precisely correct, we can imagine the graph on the left having colimits freely adjoined via Yoneda embedding into the graph on the right (colimits are represented by blue nodes and arrows) and then selecting some of those colimits for inclusion in the category of structures representing {\it global} biological information. Note that the category of global information structures in this picture is not actually cocomplete as  $\mathcal{L}_1$ is assumed to be in Figure \ref{fig:ascent}}
