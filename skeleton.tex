...
The preceding definitions are standard category theoretic constructions. Ellerman has proposed an interpretation of adjoint functors, that demonstrates their relevance to the concept of information encoding and decoding in the context of biological systems \cite{Ellerman2005}.
...
$$
hom_{\mathcal{C}}(-,-): \mathcal{C}^{opp} \times \mathcal{C} \rightarrow \textit{Sets}.
$$
...
For a hom-functor $hom_{\mathcal{C}}(-,-): \mathcal{C}^{opp} \times \mathcal{C} \rightarrow \textit{Sets}$ for $c \in \Ob(\mathcal{C})$ a covariant and contravariant functor can be derived by specializing the hom-functor to morphisms out of or into the object $c$ as
\begin{eqnarray*}
h^c \equiv hom_{\mathcal{C}}(c,-) &:& \mathcal{C} \rightarrow \textit{Sets},\\
h_c \equiv hom_{\mathcal{C}^{opp}}(-,c) &:& \mathcal{C}^{opp} \rightarrow \textit{Sets}.
\end{eqnarray*}
